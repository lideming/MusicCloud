...
import { api } from "./Api";
import { Callbacks, Action } from "../ui/utils/view";
...
export const msgcli = new (class {
  ...
  init() {
    user.onSwitchedUser.add(() => {
      if (this.connected && api.defaultAuth) this.login(api.defaultAuth!);
    });
    this.onConnected.add(() => {
      if (user.state === "logged") this.login(api.defaultAuth!);
  ...
  listenEvent(
    evt: string | { events?: string[]; remove?: string[] },
    callback: Action<any>,
    autoRetry?: boolean
  ) {
