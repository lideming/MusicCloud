...
class DataBackedListView<
  T extends DataBackedListViewItem,
  TData
> extends ListView<T> {
...
export class ListContentView extends ContentView {
  ...
  get rendered() {
    return this.domCreated;
  }

  private _canMultiSelect: boolean;
  public get canMultiSelect(): boolean {
  ...
  protected appendHeader() {
    ...
    this.header.actions.addView(
      (this.refreshBtn = new ActionBtn({ text: () => I`Refresh` }))
    );
    this.header.actions.addView(
      (this.selectAllBtn = new ActionBtn({ text: () => I`Select all` }))
    );
    this.header.actions.addView(
      (this.selectBtn = new ActionBtn({ text: () => I`Select` }))
    );
    this.selectBtn.onActive.add(() => {
      this.listView.selectionHelper.enabled =
        !this.listView.selectionHelper.enabled;
    });
    this.selectAllBtn.onActive.add(() => {
      this.listView.forEach((x) =>
        this.listView.selectionHelper.toggleItemSelection(x, true)
      );
    });
  ...
  protected appendListView() {
    ...
    this.listView.selectionHelper.onEnabledChanged.add(() => {
      this.selectBtn.hidden =
        !this.canMultiSelect && !this.listView.selectionHelper.enabled;
      this.selectBtn.text = this.listView.selectionHelper.enabled
        ? () => I`Cancel`
        : () => I`Select`;
  ...
  onDomInserted() {
    ...
    if (this.scrollBox && this._scrollPos) {
      this.listView.dom.style.minHeight =
        this._scrollPos + this.scrollBox.dom.offsetHeight + "px";
  ...
  updateView() {
    ...
    if (this.listView.length === 0) {
      if (!this.loadingIndicator) {
        this.emptyIndicator =
          this.emptyIndicator ||
          new LoadingIndicator({ state: "normal", content: I`(Empty)` });
        this.useLoadingIndicator(this.emptyIndicator);
      }
    } else {
      if (
        this.emptyIndicator &&
        this.loadingIndicator === this.emptyIndicator
      ) {
