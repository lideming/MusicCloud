...
import * as views from "../components/ui-views";
import { MainContainer, SettingsBtn, SidebarToggle } from "../components/ui-views";
...
export const ui = new (class {
  ...
  sidebar = new (class {
    ...
    get hide() {
      return this._hide && this._float;
    }

    private _btnShown = false;
    get btnShown() {
      return this._btnShown;
    }
    ...
    private _getWidth() {
      return this.dom.offsetWidth - 200;
    }
    private _movingPos: number | null = null;
    private get movingPos() {
    ...
    initPanHandler() {
      ...
      this._panxHandler.onStart.add(() => {
        ...
        this.movingPos =
          this.dom.getBoundingClientRect().left + this._leftPadding;
      });
      var lastDelta = 0;
      ...
      if (showOverlay != !!this.overlay) {
        if (showOverlay) {
          this.overlay = new Overlay({
            tag: "div.overlay",
            style: "z-index: 99; animation: none; transition: opacity .3s;",
  ...
  notification = new (class {
    ...
    get config() {
      return this.siNotification.data;
    }
    ...
    init() {
      if (
        this.config.enabled &&
        (!this.browserSupport() || !this.checkPermission())
      ) {
      ...
      playerCore.onTrackChanged.add(() => {
        if (
          ...
          this.show(track.name, {
            ...
            icon: !track?.picurl ? undefined : api.processUrl(track.picurl)!,
            silent: true,
          });
        }
      });
    ...
    async setEnable(enable: boolean) {
      if (
        enable &&
        !(this.browserSupport() && (await this.requestPermission()))
      )
        return;
  ...
  showContextMenuForItem(
    items: ListViewItem[],
    menu: ContextMenu,
    ...args: Parameters<ContextMenu["show"]>
  ) {
    menu.show(...args);
