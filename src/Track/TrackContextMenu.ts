import { ContextMenu, MenuItem, I, formatFileSize, MenuLinkItem, i18n, formatDuration, arraySum, MenuInfoItem } from "@yuuza/webfx";
...
export const trackContextMenu = (selected: Track[], ev: MouseEvent, trackViewItems?: TrackViewItem[]) => {
    ev.preventDefault();
    var m = new ContextMenu();
    const item = selected[0];
    if (selected.length == 1) {
        if (item.id && user.state != 'none' && user.serverOptions.trackCommentsEnabled !== false) m.add(new MenuItem({
        ...
        if (settings.showDownloadOptions && item.url) {
            ...
            files.forEach(f => {
                ...
                else if (item.canEdit) m.add(new MenuItem({
    ...
    const nonSameGroupItems = selected.filter(x => x != item && x.groupId !== targetGroup);
    ...
    if (item.canEdit) [0, 1].forEach(visi => {
        var count = 0;
    ...
    if (selected.length == 1) {
        if (item.visibility == 1) m.add(new CopyMenuItem({
            text: I`Copy link`,
    ...
    if (trackViewItems) {
        ...
        if (selected.length == 1
            ...
            m.add(new MenuItem({
                ...
                onActive: () => trackViewItem.actionHandler!.onTrackRemove?.([trackViewItem])
            }));
        }
        if (trackViewItem.actionHandler?.onTrackRemove && selected.length > 1
