...
export class ListIndex {
  ...
  init() {
    playerCore.onTrackChanged.add(() => {
      ...
      if (curPlaying != this.playing) {
        if (curPlaying?.id)
          this.getViewItem(curPlaying.id)?.updateWith({ playing: true });
        if (this.playing?.id)
          this.getViewItem(this.playing.id)?.updateWith({ playing: false });
    ...
    this.section.addAction(
      jsxBuild<SectionAction>(
        jsx(
          SectionAction,
          {
            onActive: () => {
      ...
      (ev) => {
        if (ev.eventPhase === Event.AT_TARGET) {
          ...
          if (
            ...
            firstHeader.offsetTop + firstHeader.offsetHeight
          ) {
  ...
  addListInfo(listinfo: Api.TrackListInfo) {
    ...
    if (
      curContent instanceof TrackListView &&
      curContent.list?.id === listinfo.id
    )
      ui.sidebarList.setActive(item);
    return item;
  }
  ...
  async newTracklist() {
    if (!(await user.waitLogin(false))) {
      this._toastLogin =
        this._toastLogin || new Toast({ text: I`Login to create playlists.` });
...
export class ListIndexViewItem extends SidebarItem {
  ...
  protected createDom(): BuildDomExpr {
    return {
      ...
      child: [
        ...
        {
          tag: "span.name.flex-1",
          text: () => this.listInfo?.name ?? this.text,
        },
      ],
    };
  }
  updateDom() {
    ...
    var tagText =
      !this.listInfo || this.listInfo.visibility == 0
        ? ""
        : this.listInfo.owner == user.id && this.listInfo.visibility == 1
        ? I`my_visibility_1`
