...
class SearchView extends ListContentView {
  ...
  async performSearch(query: string) {
    ...
    if (this.searchbar.value != query) this.searchbar.value = query;
    var li = new LoadingIndicator();
    this.useLoadingIndicator(li);
    router.nav(["search", query]);
    try {
      var r = (await api.get("tracks?query=" + encodeURIComponent(query))) as {
        tracks: Api.Track[];
      };
      this.tempList = null;
      search.checkPlaying();
