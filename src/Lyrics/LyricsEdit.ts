...
class EditableLyricsView extends LyricsView {
  ...
  getSpans(span?: SpanView | null, go?: "here" | "forward" | "backward") {
    if (!span) {
      if (this.nextSpans.length) {
        span =
          this.nextSpans[go === "backward" ? 0 : this.nextSpans.length - 1];
